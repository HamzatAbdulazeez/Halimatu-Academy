const translations = {
  en: {
    dir: "ltr",
    banner: {
      slides: [
        {
          title: "Zad Academy Program",
          subtitle: "Learn the Basics of Islam",
          description:
            "Embark on a 2-year learning journey with Zad Academy's free online program.",
          badge: "Featured Program",
          ctaPrimary: "Enroll Now",
          ctaSecondary: "Learn More",
          image: "https://res.cloudinary.com/ddj0k8gdw/image/upload/v1768633927/Halimatu-Academy-Images/Islamic-Education-1_eufmf8.jpg",
          gradient: "from-emerald-600 to-teal-700"
        },
        {
          title: "Comprehensive Curriculum",
          subtitle: "Structured Islamic Education",
          description:
            "Our carefully designed curriculum covers Quran, Fiqh, Seerah, and Ethics.",
          badge: "Featured Program",
          ctaPrimary: "Enroll Now",
          ctaSecondary: "Learn More",
          image: "https://res.cloudinary.com/ddj0k8gdw/image/upload/v1768632270/Halimatu-Academy-Images/images-4-1_r7jgos.jpg",
          gradient: "bg-[#004aad] to-[#101e55]"
        },
        {
          title: "Learn at Your Pace",
          subtitle: "Flexible Online Learning",
          description:
            "Study anytime with recorded lectures and live scholar sessions.",
          badge: "Featured Program",
          ctaPrimary: "Enroll Now",
          ctaSecondary: "Learn More",
          image: "https://res.cloudinary.com/ddj0k8gdw/image/upload/v1768634979/Halimatu-Academy-Images/q_bzkmjv.jpg",
          gradient: "from-purple-600 to-pink-700"
        }
      ]
    },

    stats: {
      duration: "4 semesters (2 years)",
      durationLabel: "Program Duration",
      semesterLength: "12 weeks",
      semesterLengthLabel: "Semester Length",
      cost: "Free",
      costLabel: "Online",
      perWeek: "Per Week",
      perWeekLabel: "15 Hours",
      startDate: "18 January",
      startDateLabel: "Start of Next Semester",
      registrationStarts: "44 Days",
      registrationStartsLabel: "Registration starts in"
    },

    menu: {
      home: "Home",
      about: "About Us",
      studyPlan: "Study Plan",
      curriculum: "Curriculum",
      schedule: "Schedule",
      knowledgeSeries: "Knowledge Series",
      contact: "Contact Us",
      login: "Login",
      register: "Register"
    },

    about: {
      aboutImageAlt: "Students learning",
      aboutTitle: "Transforming Lives Through Islamic Education",
      aboutDescription: "HSA Academy is a pioneering online Islamic education platform dedicated to making authentic Islamic knowledge accessible to Muslims worldwide.",
      featureCurriculumTitle: "Authentic Curriculum",
      featureCurriculumDesc: "Based on classical Islamic texts and teachings",
      featureFlexibleTitle: "Flexible Learning",
      featureFlexibleDesc: "Study at your own pace, anytime, anywhere",
      ctaDiscover: "Discover Our Story"
    },
    aboutstats: {
      activeStudents: "100+",
      activeStudentsLabel: "Active Students",
      expertScholars: "10+",
      expertScholarsLabel: "Expert Scholars"
    },

    pages: {
      homeTitle: "Welcome to Zad Academy"
    }
    ,
    programSection: {
      headerBadge: "Islamic Curriculum",
      title: "The Program",
      description:
        "HSA Academy provides a comprehensive Islamic curriculum consisting of seven core subjects, designed to foster a deeper understanding of Islam.",
      subjects: [
        {
          title: "Arabic Language",
          description:
            "Master the fundamentals of Arabic grammar and vocabulary to better understand the Quran and Islamic texts.",
          gradient: "from-purple-500 to-indigo-600",
        },
        {
          title: "Quranic",
          description:
            "Learn the etiquettes and moral character taught in Islam, developing excellence in behavior and interpersonal relations.",
          gradient: "from-pink-500 to-rose-600",
        },
      ],
      learnMore: "Learn More",
    },
    islamicQuote: {
      messenger: "The Messenger of Allah ﷺ said:",
      quote:
        "“Whoever takes a path upon which to obtain knowledge, Allah makes the path to Paradise easy for him.”",
      source: "Sahih Muslim",
    },
    learningPath: {
      header: "Learning Path",
      description:
        "Enroll in HSA Academy's free online program and begin your journey.",
      button: "Find out more",
      steps: [
        {
          number: "01",
          title: "Select a Language and Register",
          description: "Register and select your preferred language for the program."
        },
        {
          number: "02",
          title: "Get Resources",
          description: "Access program resources while you wait for the semester to begin."
        },
        {
          number: "03",
          title: "Participate and Collaborate Weekly",
          description: "Participate in lectures, forums, and weekly assessments throughout the semester."
        },
        {
          number: "04",
          title: "Take Final Exams",
          description: "Progress through the program by passing final exams each semester."
        },
        {
          number: "05",
          title: "Get the Certificate",
          description: "Complete all semesters successfully and receive your certificate of completion."
        }
      ]
    },
    testimonialsFAQs: {
      headerTestimonials: "What Our Students Say",
      subheaderTestimonials: "Join thousands of satisfied students who have transformed their lives through Islamic education",
      tagTestimonials: "Student Success Stories",
      headerFAQs: "Frequently Asked Questions",
      subheaderFAQs: "Everything you need to know about HSA Academy",
      tagFAQs: "Got Questions?",
      testimonials: [
        {
          name: "Amina Hassan",
          location: "Lagos, Nigeria",
          rating: 5,
          text: "HSA Academy has transformed my understanding of Islam. The structured curriculum and knowledgeable instructors made learning accessible and enjoyable.",
          course: "Year 1 Graduate",
          image: "https://images.pexels.com/photos/8121852/pexels-photo-8121852.jpeg"
        },
        {
          name: "Hamzat Abdullah",
          location: "Abuja, Nigeria",
          rating: 5,
          text: "The flexibility of online learning combined with authentic Islamic knowledge is unmatched. I can study at my own pace while balancing work and family.",
          course: "Year 2 Student"
          ,
          image: "https://i0.pickpik.com/photos/43/874/346/arabs-face-orient-arabic-preview.jpg"
        },
        {
          name: "Fatima Ahmed",
          location: "Ibadan, Nigeria",
          rating: 5,
          text: "The community aspect is incredible. I've connected with Muslims worldwide, and the weekly discussions have deepened my faith.",
          course: "Year 1 Student"
          ,
          image: "https://images.pexels.com/photos/5125239/pexels-photo-5125239.jpeg"
        },
        {
          name: "Ibrahim Mohamed",
          location: "Ilorin, Nigeria",
          rating: 5,
          text: "As a revert, this program gave me the foundation I needed. The curriculum is comprehensive, and the support is amazing.",
          course: "Year 2 Graduate"
          ,
          image: "https://images.pexels.com/photos/8303314/pexels-photo-8303314.jpeg"
        },
        {
          name: "Khadija Ali",
          location: "Osogbo, Nigeria",
          rating: 5,
          text: "Best decision I've made for my Islamic education. The quality of teaching and resources is outstanding.",
          course: "Year 1 Graduate"
          ,
          image: "https://images.rawpixel.com/image_800/czNmcy1wcml2YXRlL3Jhd3BpeGVsX2ltYWdlcy93ZWJzaXRlX2NvbnRlbnQvbHIvZmw0MDQ3MTY4NDg0MC1pbWFnZS1rdWRpMzRzNS5qcGc.jpg"
        },
        {
          name: "Hassan Khan",
          location: "Zaria, Nigeria",
          rating: 5,
          text: "The teachers are knowledgeable and patient. I've learned so much about the Quran and Hadith in just one year!",
          course: "Year 1 Student"
          ,
          image: "https://images.rawpixel.com/image_800/czNmcy1wcml2YXRlL3Jhd3BpeGVsX2ltYWdlcy93ZWJzaXRlX2NvbnRlbnQvbHIvZmw0MjM2NTAwNzQ0MS1pbWFnZS1rcHFvcHR4cy5qcGc.jpg"
        },
        {
          name: "Aisha Rahman",
          location: "Kano, Nigeria",
          rating: 5,
          text: "The Arabic language course helped me understand the Quran in its original language. Truly transformative experience!",
          course: "Year 2 Student"
          ,
          image: "https://images.pexels.com/photos/8303314/pexels-photo-8303314.jpeg"
        }
      ],
      faqs: [
        {
          question: "Is the program really free?",
          answer: "Yes! HSA Academy is completely free for all students. We believe Islamic education should be accessible to everyone, regardless of financial circumstances. There are no hidden fees or charges."
        },
        {
          question: "What are the language options available?",
          answer: "We offer the program in multiple languages including English, Arabic, French, Urdu, and Turkish. You can select your preferred language during registration and switch if needed."
        },
        {
          question: "How much time do I need to dedicate each week?",
          answer: "We recommend dedicating approximately 15 hours per week to get the most out of the program. This includes watching lectures, reading materials, participating in forums, and completing assignments. However, the program is flexible and you can study at your own pace."
        },
        {
          question: "Do I need any prerequisites to enroll?",
          answer: "No prerequisites are required! The program is designed for students of all levels, from beginners to those with some Islamic knowledge. Our curriculum starts with fundamentals and progressively builds your understanding."
        }
      ]
    },
    footer: {
      stayUpdated: "Stay Updated",
      subscribeDescription: "Subscribe to our newsletter for the latest updates and Islamic insights",
      placeholderEmail: "Enter your email",
      subscribeButton: "Subscribe",
  
      aboutTitle: "HSA Academy",
      aboutDescription: "Empowering Muslims worldwide with authentic Islamic education. Learn at your own pace with renowned scholars.",
      followUs: "Follow Us",
  
      quickLinksTitle: "Quick Links",
      quickLinks: [
        { name: "About Us", href: "#" },
        { name: "The Program", href: "#" },
        { name: "Learning Path", href: "#" },
        { name: "Contact", href: "#" },
      ],
  
      resourcesTitle: "Resources",
      resources: [
        { name: "Student Portal", href: "#" },
        { name: "FAQs", href: "#" },
        { name: "Study Materials", href: "#" },
      ],
  
      contactTitle: "Contact Us",
      contact: [
        { icon: "Mail", text: "info@hsacademy.com", href: "mailto:info@hsacademy.com" },  // updated domain to match academy name
        { icon: "Phone", text: "+234 800 HSA ACADEMY", href: "tel:+2348004722233" },     // example Nigerian format
        { icon: "MapPin", text: "Port Harcourt, Rivers State, Nigeria", href: "#" }      // localized
      ],
  
      copyright: (year) => `© ${year} HSA Academy. Made with ❤️ for the Muslim Ummah`,
      privacyPolicy: "Privacy Policy",
      termsOfService: "Terms of Service",
      cookiePolicy: "Cookie Policy"
    }
  },

  ar: {
    dir: "rtl",
    banner: {
      slides: [
        {
          title: "برنامج أكاديمية زاد",
          subtitle: "تعلم أساسيات الإسلام",
          description:
            "ابدأ رحلة تعليمية لمدة عامين مع برنامج أكاديمية زاد المجاني عبر الإنترنت.",
          badge: "برنامج مميز",
          ctaPrimary: "سجل الآن",
          ctaSecondary: "اعرف المزيد",
          image: "https://res.cloudinary.com/ddj0k8gdw/image/upload/v1768633927/Halimatu-Academy-Images/Islamic-Education-1_eufmf8.jpg",
          gradient: "from-emerald-600 to-teal-700"
        },
        {
          title: "منهج شامل",
          subtitle: "تعليم إسلامي منظم",
          description:
            "يغطي المنهج المصمم بعناية القرآن والفقه والسيرة والأخلاق.",
          badge: "برنامج مميز",
          ctaPrimary: "سجل الآن",
          ctaSecondary: "اعرف المزيد",
          image: "https://res.cloudinary.com/ddj0k8gdw/image/upload/v1768632270/Halimatu-Academy-Images/images-4-1_r7jgos.jpg",
          gradient: "bg-[#004aad] to-[#101e55]"
        },
        {
          title: "تعلم بالوتيرة التي تناسبك",
          subtitle: "تعلم مرن عبر الإنترنت",
          description:
            "ادرس في أي وقت من خلال المحاضرات المسجلة والجلسات المباشرة مع العلماء.",
          badge: "برنامج مميز",
          ctaPrimary: "سجل الآن",
          ctaSecondary: "اعرف المزيد",
          image: "https://res.cloudinary.com/ddj0k8gdw/image/upload/v1768634979/Halimatu-Academy-Images/q_bzkmjv.jpg",
          gradient: "from-purple-600 to-pink-700"
        }
      ]
    },

    stats: {
      duration: "4 فصول (سنتان)",
      durationLabel: "مدة البرنامج",
      semesterLength: "12 أسبوعًا",
      semesterLengthLabel: "مدة الفصل",
      cost: "مجاني",
      costLabel: "عبر الإنترنت",
      perWeek: "بالأسبوع",
      perWeekLabel: "15 ساعة",
      startDate: "18 يناير",
      startDateLabel: "بداية الفصل القادم",
      registrationStarts: "44 يومًا",
      registrationStartsLabel: "بداية التسجيل في"
    },

    menu: {
      home: "الرئيسية",
      about: "من نحن",
      studyPlan: "الخطة الدراسية",
      curriculum: "المنهج",
      schedule: "الجدول",
      knowledgeSeries: "سلسلة المعرفة",
      contact: "اتصل بنا",
      login: "تسجيل الدخول",
      register: "إنشاء حساب"
    },

    pages: {
      homeTitle: "مرحبًا بك في أكاديمية زاد"
    },

    about: {
      aboutImageAlt: "الطلاب يتعلمون",
      aboutTitle: "تحويل الحياة من خلال التعليم الإسلامي",
      aboutDescription: "أكاديمية HSA هي منصة تعليم إسلامي عبر الإنترنت تهدف إلى جعل المعرفة الإسلامية الأصيلة متاحة للمسلمين في جميع أنحاء العالم.",
      featureCurriculumTitle: "منهج أصيل",
      featureCurriculumDesc: "مبني على النصوص والتعليمات الإسلامية الكلاسيكية",
      featureFlexibleTitle: "تعلم مرن",
      featureFlexibleDesc: "ادرس حسب وتيرتك الخاصة، في أي وقت وفي أي مكان",
      ctaDiscover: "اكتشف قصتنا"
    },
    aboutstats: {
      activeStudents: "100+",
      activeStudentsLabel: "الطلاب النشطون",
      expertScholars: "10+",
      expertScholarsLabel: "العلماء الخبراء"
    },

    programSection: {
      headerBadge: "المنهج الإسلامي",
      title: "البرنامج",
      description:
        "توفر أكاديمية HSA منهجًا إسلاميًا شاملاً يتكون من سبعة مواضيع أساسية، مصممة لتعزيز فهم أعمق للإسلام.",
      subjects: [
        {
          title: "اللغة العربية",
          description:
            "إتقان أساسيات قواعد ومفردات اللغة العربية لفهم أفضل للقرآن والنصوص الإسلامية.",
          gradient: "from-purple-500 to-indigo-600",
        },
        {
          title: "القرآن الكريم",
          description:
            "تعلم آداب وأخلاق الإسلام، وتطوير التميز في السلوك والعلاقات بين الناس.",
          gradient: "from-pink-500 to-rose-600",
        },
      ],
      learnMore: "اعرف المزيد",
    },

    islamicQuote: {
      messenger: "قال رسول الله ﷺ:",
      quote:
        "“من سلك طريقا يطلب فيه العلم سهل الله له به طريقا إلى الجنة.”",
      source: "صحيح مسلم",
    },

    learningPath: {
      header: "مسار التعلم",
      description: "سجل في البرنامج المجاني لأكاديمية HSA وابدأ رحلتك.",
      button: "اكتشف المزيد",
      steps: [
        {
          number: "01",
          title: "اختر اللغة وسجل",
          description: "سجل واختر لغتك المفضلة للبرنامج."
        },
        {
          number: "02",
          title: "احصل على الموارد",
          description: "الوصول إلى موارد البرنامج أثناء انتظار بدء الفصل الدراسي."
        },
        {
          number: "03",
          title: "شارك وتعاون أسبوعيًا",
          description: "شارك في المحاضرات والمنتديات والتقييمات الأسبوعية طوال الفصل الدراسي."
        },
        {
          number: "04",
          title: "اجتياز الاختبارات النهائية",
          description: "تقدم في البرنامج من خلال اجتياز الاختبارات النهائية في كل فصل."
        },
        {
          number: "05",
          title: "الحصول على الشهادة",
          description: "أكمل جميع الفصول الدراسية بنجاح واحصل على شهادة إتمام البرنامج."
        }
      ]
    },
    testimonialsFAQs: {
      headerTestimonials: "ماذا يقول طلابنا",
      subheaderTestimonials: "انضم إلى آلاف الطلاب الراضين الذين غيّروا حياتهم من خلال التعليم الإسلامي",
      tagTestimonials: "قصص نجاح الطلاب",
      headerFAQs: "الأسئلة الشائعة",
      subheaderFAQs: "كل ما تحتاج معرفته عن أكاديمية HSA",
      tagFAQs: "لديك أسئلة؟",
      testimonials: [
        {
          name: "أمينة حسن",
          location: "لاغوس، نيجيريا",
          rating: 5,
          text: "حوّلت أكاديمية HSA فهمي للإسلام. المنهج المنظم والمدرسون ذوو المعرفة جعلوا التعلم سهلاً وممتعاً.",
          course: "خريج السنة الأولى"
          ,
          image: "https://images.pexels.com/photos/8121852/pexels-photo-8121852.jpeg"
        },
        {
          name: "حمزة عبدالله",
          location: "أبوجا، نيجيريا",
          rating: 5,
          text: "مرونة التعلم عبر الإنترنت مع المعرفة الإسلامية الأصيلة لا مثيل لها. أستطيع الدراسة بوتيرتي مع موازنة العمل والعائلة.",
          course: "طالب السنة الثانية"
          ,
          image: "https://i0.pickpik.com/photos/43/874/346/arabs-face-orient-arabic-preview.jpg"
        },
        {
          name: "فاطمة أحمد",
          location: "إبادان، نيجيريا",
          rating: 5,
          text: "الجانب المجتمعي رائع. تواصلت مع مسلمين من جميع أنحاء العالم، والمناقشات الأسبوعية عمّقت إيماني.",
          course: "طالبة السنة الأولى"
          ,
          image: "https://images.pexels.com/photos/5125239/pexels-photo-5125239.jpeg"
        },
        {
          name: "إبراهيم محمد",
          location: "إلورين، نيجيريا",
          rating: 5,
          text: "كمسلم جديد، أعطاني هذا البرنامج الأساس الذي كنت بحاجة إليه. المنهج شامل، والدعم مذهل.",
          course: "خريج السنة الثانية"
          ,
          image: "https://images.pexels.com/photos/8303314/pexels-photo-8303314.jpeg"
        },
        {
          name: "خديجة علي",
          location: "أوسوغبو، نيجيريا",
          rating: 5,
          text: "أفضل قرار اتخذته في تعليمي الإسلامي. جودة التدريس والموارد ممتازة.",
          course: "خريجة السنة الأولى"
          ,
          image: "https://images.rawpixel.com/image_800/czNmcy1wcml2YXRlL3Jhd3BpeGVsX2ltYWdlcy93ZWJzaXRlX2NvbnRlbnQvbHIvZmw0MDQ3MTY4NDg0MC1pbWFnZS1rdWRpMzRzNS5qcGc.jpg"
        },
        {
          name: "حسن خان",
          location: "زاريا، نيجيريا",
          rating: 5,
          text: "المدرسون على دراية وصبورون. تعلمت الكثير عن القرآن والحديث في سنة واحدة فقط!",
          course: "طالب السنة الأولى"
          ,
          image: "https://images.rawpixel.com/image_800/czNmcy1wcml2YXRlL3Jhd3BpeGVsX2ltYWdlcy93ZWJzaXRlX2NvbnRlbnQvbHIvZmw0MjM2NTAwNzQ0MS1pbWFnZS1rcHFvcHR4cy5qcGc.jpg"
        },
        {
          name: "عائشة رحمن",
          location: "كانو، نيجيريا",
          rating: 5,
          text: "دورة اللغة العربية ساعدتني على فهم القرآن بلغته الأصلية. تجربة تحويلية حقاً!",
          course: "طالبة السنة الثانية"
          ,
          image: "https://images.pexels.com/photos/8303314/pexels-photo-8303314.jpeg"
        }
      ],
      faqs: [
        {
          question: "هل البرنامج مجاني فعلاً؟",
          answer: "نعم! أكاديمية HSA مجانية تماماً لجميع الطلاب. نؤمن أن التعليم الإسلامي يجب أن يكون متاحاً للجميع بغض النظر عن الظروف المالية. لا توجد رسوم خفية."
        },
        {
          question: "ما هي اللغات المتوفرة؟",
          answer: "نقدم البرنامج بعدة لغات منها الإنجليزية والعربية والفرنسية والأردية والتركية. يمكنك اختيار اللغة المفضلة عند التسجيل والتبديل لاحقاً."
        },
        {
          question: "كم من الوقت أحتاج أسبوعياً؟",
          answer: "نوصي بتخصيص حوالي 15 ساعة أسبوعياً للاستفادة القصوى. يشمل ذلك مشاهدة المحاضرات، القراءة، المشاركة في المنتديات وإكمال الواجبات. البرنامج مرن ويمكنك الدراسة بوتيرتك."
        },
        {
          question: "هل هناك شروط مسبقة للتسجيل؟",
          answer: "لا يوجد أي شروط مسبقة! البرنامج مصمم لجميع المستويات، من المبتدئين إلى من لديهم بعض المعرفة الإسلامية. يبدأ المنهج بالأساسيات ويتقدم تدريجياً."
        }
      ]
    },

    footer: {
      stayUpdated: "ابقَ على اطلاع",
      subscribeDescription: "اشترك في نشرتنا الإخبارية للحصول على آخر التحديثات والرؤى الإسلامية",
      placeholderEmail: "أدخل بريدك الإلكتروني",
      subscribeButton: "اشترك",
  
      aboutTitle: "أكاديمية HSA",
      aboutDescription: "تمكين المسلمين في جميع أنحاء العالم بالتعليم الإسلامي الأصيل. تعلم بوتيرتك الخاصة مع علماء مشهورين.",
      followUs: "تابعنا",
  
      quickLinksTitle: "روابط سريعة",
      quickLinks: [
        { name: "من نحن", href: "#" },
        { name: "البرنامج", href: "#" },
        { name: "مسار التعلم", href: "#" },
        { name: "اتصل بنا", href: "#" },
      ],
  
      resourcesTitle: "الموارد",
      resources: [
        { name: "بوابة الطالب", href: "#" },
        { name: "الأسئلة الشائعة", href: "#" },
        { name: "مواد الدراسة", href: "#" },
      ],
  
      contactTitle: "اتصل بنا",
      contact: [
        { icon: "Mail", text: "info@hsacademy.com", href: "mailto:info@hsacademy.com" },
        { icon: "Phone", text: "+234 800 HSA ACADEMY", href: "tel:+2348004722233" },
        { icon: "MapPin", text: "بورت هاركورت، ولاية ريفرز، نيجيريا", href: "#" }
      ],
  
      copyright: (year) => `© ${year} أكاديمية HSA. صُنع بحب ❤️ للأمة الإسلامية`,
      privacyPolicy: "سياسة الخصوصية",
      termsOfService: "شروط الخدمة",
      cookiePolicy: "سياسة ملفات تعريف الارتباط"
    }

  }
};

export default translations;
